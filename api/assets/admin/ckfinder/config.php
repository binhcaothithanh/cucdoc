...
function CheckAuthentication() {
    return true;
    define('BASEPATH', ''); //echo $_SERVER['DOCUMENT_ROOT'];exit; 
    require $_SERVER['DOCUMENT_ROOT'] . '/../admincp/config/config.php';
    $session = $_COOKIE[$config['sess_cookie_name']];
    if ($session === FALSE) {
    ...
    if ($config['sess_encrypt_cookie'] == TRUE) {
        ...
        if ($hash !== md5($session . $config['encryption_key'])) {
            log_message('error', 'The session cookie data did not match what was expected. This could be a possible hacking attempt.');
            unset($_COOKIE[$config['sess_cookie_name']]);
            return FALSE;
        }
    }

    // Unserialize the session array
    $session = unserializea($session);
    ...
    if (($session['last_activity'] + $config['sess_expiration'] ) < time()) {
        unset($_COOKIE[$config['sess_cookie_name']]);
        return FALSE;
    }

    // Does the IP Match?
//    if ($config['sess_match_ip']  == TRUE AND $session['ip_address'] != $_SERVER['SERVER_ADDR']) {
//        unset ($_COOKIE[$config['sess_cookie_name']]);
//        return FALSE;
//    }
    // Does the User Agent Match?

    if ($session['can_access_backend']) {
        return true;
    } else {
        return FALSE;
    }
 ...
 */
$baseUrl = 'http://' . $_SERVER['SERVER_NAME'] . '/assets/';
...
$config['ResourceType'][] = Array(
    ...
    'url' => $baseUrl . 'dl',
    'directory' => $baseDir . 'dl',
    'maxSize' => 0,
    'allowedExtensions' => 'jar,jad',
    'deniedExtensions' => '');
